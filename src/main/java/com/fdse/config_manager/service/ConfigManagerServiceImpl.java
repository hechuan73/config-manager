...
import com.fdse.config_manager.async.AsyncTask;
import com.fdse.config_manager.bean.*;
import com.fdse.config_manager.bean.Metrics.Common.V1beta1ItemsUsage;
import com.fdse.config_manager.bean.Metrics.NodeMetrics.V1beta1NodeItem;
import com.fdse.config_manager.bean.Metrics.NodeMetrics.V1beta1NodeList;
import com.fdse.config_manager.bean.Metrics.NodeMetrics.NodeMetrics;
import com.fdse.config_manager.bean.Metrics.PodMetrics.PodMetrics;
import com.fdse.config_manager.bean.Metrics.PodMetrics.V1beta1Container;
import com.fdse.config_manager.bean.Metrics.PodMetrics.V1beta1PodItem;
import com.fdse.config_manager.bean.Metrics.PodMetrics.V1beta1PodList;
...
import com.fdse.config_manager.util.Cluster;
import com.fdse.config_manager.util.MyConfig;
import com.fdse.config_manager.util.RemoteExecuteCommand;
...
//        rec.uploadFile(svcLongDelayFilePath);
    ...
    @Override
    public AsyncRequestSequenceResponse setAsyncRequestsSequence(SetAsyncRequestSequenceRequest setAsyncRequestSequenceRequest){
        ...
        AsyncRequestSequenceResponse response = new AsyncRequestSequenceResponse(true," setAsyncRequestsSequence Complete");
        return response;
    }

    @Override
    public AsyncRequestSequenceResponse setAsyncRequestsSequenceWithSource(AsyncRequestSequenceRequestWithSource request){
        Cluster cluster = getClusterByName(request.getClusterName());
        System.out.println(String.format("The cluster to operate is [%s]", cluster.getName()));
    ...
    @Override
    public AsyncRequestSequenceResponse unsuspendAllRequest(AsyncRequestSequenceRequestWithSource request){
        Cluster cluster = getClusterByName(request.getClusterName());
        System.out.println(String.format("The cluster to operate is [%s]", cluster.getName()));
        ArrayList<String> svcList = request.getSvcList();
        String resultStr = "";
        ...
        return new AsyncRequestSequenceResponse(true,resultStr);

    }

    @Override
    public AsyncRequestSequenceResponse setAsyncRequestSequenceWithSrcCombineWithFullSuspend(AsyncRequestSequenceRequestWithSource request){
        Cluster cluster = getClusterByName(request.getClusterName());
        System.out.println(String.format("The cluster to operate is [%s]", cluster.getName()));
        String str = "";
        ...
        return new AsyncRequestSequenceResponse(true,str);
...
//        }
    ...
    public SetRunOnSingleNodeResponse setRunOnSingleNode(String clusterName) {
        ...
        if(nodeList.getItems() != null && nodeList.getItems().size() > 0){
            for(V1Node node : nodeList.getItems()){
                ...
                if(node.getSpec().getTaints() == null) { workingNodeList.add(node); }
            }
        }else{
    ...
    public GetNodesListResponse getNodesList(String clusterName) {
        ...
        for(V1Node node : nodeList.getItems()){
            ...
            System.out.println(String.format("The node name is %s and the role is %s",node.getMetadata().getName(),node.getSpec().getTaints() == null?"Minion":"Master"));
            //Set the role
            if(node.getSpec().getTaints() != null) { nodeInfo.setRole("Master"); }
            else { nodeInfo.setRole("Minion");}
            //Set the name
    ...
    private V1beta1ItemsUsage getPodMetricsUsage(V1beta1PodItem pod) {
        ...
        for (V1beta1Container container : pod.getContainers()) {
            cpu = cpu.add(container.getUsage().getCpu().contains("m")
                    ? new BigDecimal(container.getUsage().getCpu().split("m")[0]).divide(new BigDecimal(1000), 3, RoundingMode.HALF_UP)
